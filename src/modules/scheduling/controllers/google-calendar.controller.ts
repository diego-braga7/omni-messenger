import { Controller, Get, Param, Query, Res, Logger, HttpStatus } from '@nestjs/common';
...
export class GoogleCalendarController {
  ...
  async auth(@Param('professionalId') professionalId: string, @Res() res: Response) {
    const professional = await this.professionalRepo.findOne({ where: { id: professionalId } });
    if (!professional) {
        return res.status(HttpStatus.NOT_FOUND).json({ message: 'Professional not found' });
    }
  ...
  async callback(@Query('code') code: string, @Query('state') professionalId: string, @Res() res: Response) {
    if (!code || !professionalId) {
        return res.status(HttpStatus.BAD_REQUEST).json({ message: 'Missing code or state' });
    }
    ...
    try {
        ...
        return res.status(HttpStatus.OK).json({ message: 'Google Calendar authentication successful' });
    } catch (error) {
